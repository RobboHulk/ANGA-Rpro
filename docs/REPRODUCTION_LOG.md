...
- **结果**：
  ...
  | 0.3 | 待补 | 待补 | 待补（首次因文件描述符耗尽在 epoch1 训练中崩溃，无 checkpoint，重跑中） |
  | 0.5 | 12 | 0.7311@epoch2 | 0.6651 |
  | 0.7（=实验2） | 13 | 0.7000@epoch3 | 0.6692 |
  | 0.9 | 11 | 0.7064@epoch1 | 0.6707 |
- **观察**：测试集 AUROC 在 0.1~0.9 缺失率区间内相对平稳（0.665~0.676），没有随缺失率单调下降，符合论文"ANGA 在各缺失率下保持稳健"的定性结论，但样本量小（每个点只有单次跑），噪声可能掩盖真实趋势。
...
恢复方式：对于"训练已完成、只是没跑完最终测试"的 6 次跑（ablation_000/100/110/101、image_0.7、k_9、rate_0.1，共 7 个），写了一个独立小脚本 `run_test_only_v2.py`——用保存下来的 `best_model.pth` 重新实例化 `Trainer`，只调用 `trainer._test()`，不用重新训练就能拿到真实的测试集 AUROC（已在文中体现）。对于 `rate_0.3` 这种没保存过 checkpoint 的，只能完整重跑。

---

## 待办 / 尚未复现

- MM-IMDb、Food101 全部场景（阶段②③代码目前硬编码只支持 HateMemes，需要先扩展这两个类支持其他数据集，见上文"额外发现"）
