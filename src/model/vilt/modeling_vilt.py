...
================================================================================
【本文件说明】
本文件是从 HuggingFace `transformers` 库复制的 ViLT（Vision-and-Language
Transformer）PyTorch 实现，非论文作者原创。在 ANGA 项目中它扮演
"预训练多模态骨干（backbone）"的角色：

    - ViltModel        : 被 ANGA 用作冻结的特征提取器（embedding + encoder）
    - ViltEmbeddings   : 负责把文本 token 与图像 patch 编码成统一的向量序列
    - ViltPooler       : 取 [CLS] 表征做 dense + tanh，得到句级表征用于分类

核心数据流（本项目实际用到的部分）：
    文本 input_ids ─┐
                    ├─→ ViltEmbeddings → 拼接向量 (B, 128+145, 768) ─→ ViltEncoder
    图像 pixel_values┘
    → LayerNorm → ViltPooler → 768 维句级表征

由于 ANGA 冻结了 ViLT 主干，本文件的绝大多数类在训练时参数不更新。
================================================================================
"""
...
class ViltEmbeddings(nn.Module):
    """
    构造文本与图像的初始嵌入（embeddings）。

    文本嵌入与 BERT 一致（word + position + token_type）。
    图像 patch 嵌入与 ViT 一致（patch 投影 + position embedding）。
    """
    ...
    def visual_embed(self, pixel_values, pixel_mask, max_image_length=200):
        """
        把原始图像像素转换为 patch 嵌入（含位置编码），并处理变长图像。

        参数：
            pixel_values     : 图像像素 (B, 3, H, W)
            pixel_mask       : 像素掩码 (B, H, W)，1=真实像素，0=padding
            max_image_length : 最多保留多少个 patch

        返回：
            x        : patch 嵌入序列（含 [CLS]）(B, n_patch+1, hidden)
            x_mask   : 对应的注意力掩码 (B, n_patch+1)
            (patch_index, (height, width)) : patch 在原图的位置索引与特征图尺寸
        """
        # 从卷积核形状推导 patch 尺寸：projection.weight 形状为 (out, in, ph, pw)
        _, _, ph, pw = self.patch_embeddings.projection.weight.shape

        # 卷积投影得到 patch 特征：(B, hidden, H/32, W/32)
        x = self.patch_embeddings(pixel_values)
        # 把像素掩码下采样到与 patch 特征图一致的分辨率
        x_mask = pixel_mask[:, None, :, :].float()
        x_mask = nn.functional.interpolate(x_mask, size=(x.shape[2], x.shape[3])).long()
        # 每个样本在高度/宽度方向上的有效 patch 数量
        x_h = x_mask[:, 0].sum(dim=1)[:, 0]
        x_w = x_mask[:, 0].sum(dim=2)[:, 0]

        batch_size, num_channels, height, width = x.shape
        patch_dim = self.config.image_size // self.config.patch_size  # 384/32 = 12
        # 把 1D 位置嵌入重排成 2D，便于对变长图像做双线性插值
        spatial_pos = self.position_embeddings[:, 1:, :].transpose(1, 2).view(1, num_channels, patch_dim, patch_dim)
        # 对每个样本按其实际尺寸 h,w 插值位置编码，并补齐到统一的 height,width
        pos_embed = torch.cat(
            [
                nn.functional.pad(
        ...
        pos_embed = pos_embed.flatten(2).transpose(1, 2)   # (B, n_patch, hidden)
        x = x.flatten(2).transpose(1, 2)                    # (B, n_patch, hidden)

        # 构造 patch 的 2D 索引，用于后续按序选取
        ...
        # 确定每个样本最多保留的 patch 数（max_image_length）
        if max_image_length < 0 or max_image_length is None or not isinstance(max_image_length, int):
            # 例如 aug 是 800x1333，最大有效分辨率就是 800x1333；
            # (800//32)*(1333//32) 是单张图像能获得的最大 patch 数
        ...
        # 分离有效 patch 与无效（padding）patch 的索引
        ...
        # 统计每个样本的有效/无效 patch 数与需要填充的 patch 数
        valid_nums = [v.size(0) for v in valid_row_idx]
        non_valid_nums = [v.size(0) for v in non_valid_row_idx]
        pad_nums = [max_image_length - v for v in valid_nums]

        # 逐样本选择保留哪些 patch：有效 patch 全保留；若不足 max_image_length
        # 则从 padding patch 中有放回地随机采样补足（保证序列长度一致）
        select = []
        for i, (v, nv, p) in enumerate(zip(valid_nums, non_valid_nums, pad_nums)):
            if p <= 0:
                # 有效 patch 数已足够：随机采样 max_image_length 个有效 patch
                valid_choice = torch.multinomial(torch.ones(v).float(), max_image_length)
                select.append(valid_row_idx[i][valid_choice])
            else:
                # 有效 patch 不足：补采 padding patch
                pad_choice = torch.multinomial(torch.ones(nv).float(), p, replacement=True)
                select.append(torch.cat([valid_row_idx[i], non_valid_row_idx[i][pad_choice]], dim=0))

        select = torch.cat(select, dim=0)
        # 按选中的 patch 索引取出对应的特征、掩码、位置编码
...
class ViltPatchEmbeddings(nn.Module):
    ...
    图像 → patch 嵌入：用一个与 patch 大小相同的卷积核做投影，
    把图像切成不重叠的 patch，并线性映射到 hidden_size 维。
    """

    def __init__(self, config):
        super().__init__()
        image_size, patch_size = config.image_size, config.patch_size   # 384, 32
        num_channels, hidden_size = config.num_channels, config.hidden_size  # 3, 768

        image_size = image_size if isinstance(image_size, collections.abc.Iterable) else (image_size, image_size)
        patch_size = patch_size if isinstance(patch_size, collections.abc.Iterable) else (patch_size, patch_size)
        # patch 总数 = (H/32) * (W/32) = 12 * 12 = 144
        ...
        # 卷积实现 patch 投影：kernel/stride 都等于 patch_size，等效于切块
...
class ViltLayer(nn.Module):
    """
    单个 Transformer 编码层（对应 timm 实现中的 Block 类）。

    ViLT 采用 pre-norm 结构：先 LayerNorm 再做自注意力，再做 LayerNorm 接 FFN，
    每个子层都有残差连接。
    """
    ...
    def __init__(self, config):
        ...
        self.seq_len_dim = 1
        self.attention = ViltAttention(config)          # 多头自注意力
        self.intermediate = ViltIntermediate(config)    # FFN 第一层（升维）
        self.output = ViltOutput(config)                # FFN 第二层（降维 + 残差）
        self.layernorm_before = nn.LayerNorm(config.hidden_size, eps=config.layer_norm_eps)  # 注意力前的 LN
        self.layernorm_after = nn.LayerNorm(config.hidden_size, eps=config.layer_norm_eps)   # FFN 前的 LN

    def forward(self, hidden_states, attention_mask=None, head_mask=None, output_attentions=False):
        # 自注意力（先 LN 后 attention）
        ...
        # 第二个残差连接在这里完成（ViltOutput.forward 内部做 + input_tensor）
...
class ViltModel(ViltPreTrainedModel):
    ...
    在 ANGA 项目中，它被加载预训练权重后作为冻结的 backbone，
    只取用其中的 embeddings / encoder / layernorm / pooler 四个子模块。
    """
    ...
    def __init__(self, config, add_pooling_layer=True):
        ...
        self.embeddings = ViltEmbeddings(config)   # 嵌入层
        self.encoder = ViltEncoder(config)         # 12 层 Transformer 编码器

        self.layernorm = nn.LayerNorm(config.hidden_size, eps=config.layer_norm_eps)
        self.pooler = ViltPooler(config) if add_pooling_layer else None  # 池化层（可选）
    ...
    def forward(
        ...
        >>> model = ViltModel.from_pretrained("/data/gzh/RAGPT-master/src/model/vilt-b32-mlm")
...
class ViltPooler(nn.Module):
    """
    池化层：取序列第一个 token（[CLS]）的隐藏状态，做 dense + tanh，
    得到用于分类/检索的句级表征。
    """
