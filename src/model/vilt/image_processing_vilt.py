...
================================================================================
【本文件说明】
从 HuggingFace `transformers` 复制的 ViLT 图像预处理器（第三方代码）。

作用：把 PIL 图像经过 resize（短边 384，长边按比例且不超过上限）、
rescale（除以 255）、normalize（ImageNet 均值/方差）、pad（补齐到 batch
内最大尺寸）等步骤，输出模型可直接使用的 pixel_values 与 pixel_mask。

在 ANGA 项目中被 Collator 与 MemoryBankGenerator 调用，用于图像预处理。
================================================================================
"""
